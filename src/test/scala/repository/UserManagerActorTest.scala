...
class UserManagerActorTest extends GherkinSpec {
  ...
  Feature ("A UserManagerActor behavior") {
    ...
    val userName = generateRandomString("user")
    ...
    Scenario("Return a Right(UserState) on a valid (different name) UpdateUser command") {
      ...
      val updatedUserName = generateRandomString("user")

      When("UpdateUser command sent")
      userManagerActor ! UpdateUser(autoGeneratedUserId, Some(updatedUserName), numGamesOwned, numReviews)

      Then("Correct UserState should be returned")
      expectMsg(Right(baseUserState.copy(userId = autoGeneratedUserId, name = Some(updatedUserName))))
