...
class ReviewRouterTest extends RoutesSpec {
  ...
  Feature("A ReviewRouter Behavior") {
    ...
    val gameRoutes: Route = GameRouter(stateManagers.Command.game, stateManagers.Query.game).routes
     ...
     When("The User sends a valid  create Review payload")
     ...
     assert(request.author.userId == autoGeneratedUserId)
     assert(request.steamApp.steamAppId == autoGeneratedSteamGameId)
    }

    Scenario("A User sends an Invalid CreateReviewRequest") {
      implicit val timeout = RouteTestTimeout(300.seconds.dilated)
      Given("a Review Information and a POST request to generate Review and valid AuthorId and GameId")

      val createReviewRequest = generateReview(Math.abs(Random.nextLong()), Math.abs(Random.nextLong()))
      When("The User sends an invalid  create Review payload with non existent gameId and authorId")
      val request = Post("/reviews")
        .withEntity(ContentTypes.`application/json`, createReviewRequest) ~!> reviewRoutes ~> check {

        Then("create review will be returned with 200 ok status")
        assert(status == StatusCodes.BadRequest)
        entityAs[ResponseWithMessage]
      }
       assert(request.statusCode == 400)
       assert(!request.message.value.isBlank)
       assert(!request.message.value.isEmpty)
       info(s"Expected exception ${request.message.value}")
    }
    ...
    Scenario("A User sends a Get Request by ReviewId") {
      ...
      When("Get request is sent for the new reviewId")
      val getRequest = Get(s"/reviews/${createRequestResponse.reviewId}") ~!> reviewRoutes ~> check {
        Then("Corresponding Review State should be returned")
        assert(status == StatusCodes.OK)
    ...
    //TODO: Failed Test Delete review seems to fail, 500 error, futures used inside actor, similar to GameManagerActor update
//    ignore("Failed Test due to use of Futures/Ask Pattern inside Receive Method.") {
      ...
      Scenario("A User sends a Delete Request for a Review") {
        ...
        val autoGeneratedSteamGameId = Post("/games")
          .withEntity(ContentTypes.`application/json`, game) ~!> gameRoutes ~> check {
          assert(status == StatusCodes.Created)
          processHeaderId(headers)
        }
...
//    }
    ...
    //TODO: Failed Test Should allow repeated reviews for the same game with same author
    Scenario("Should Not Allow multiple reviews for the same game with same author ") {

      implicit val timeout = RouteTestTimeout(300.seconds.dilated)
      Given("Repeated Review Information using same Author and SteamGameId")
      val user = generateUserRequest()
      val game = generateGameRequest()
      ...
      When("A second request to create a Review with the repeated authorId/gameId")
      val createRequestResponseRepeated = Post("/reviews")
        .withEntity(ContentTypes.`application/json`, createReviewRequest) ~!> reviewRoutes ~> check {
