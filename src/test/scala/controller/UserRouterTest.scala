...
//      ActorRef.noSender,userManagerActor,userManagerActor,ActorRef.noSender)
    ...
    Scenario("A client sends a request to get a user") {
      ...
      val x = Random.alphanumeric
      val userName = s"game_${(x take 20).mkString}"
      val user = generateUserRequest(newUserName = userName)
      ...
      Get(s"/users/$autoGeneratedId") ~!> routes ~> check {
        ...
        assert(entityAs[UserState] == baseUserState.copy(userId = autoGeneratedId, name = Some(userName)))
      }
    }
    ...
    Scenario("A Client sends a request to Update a User") {
      ...
      val updateUser = generateUserRequest(updatedUserName,updatedNumGamesOwned,updatedNumReviews)
